...
export function SubmitCandidateForm({ requisitionId, candidates }: { requisitionId: string; candidates: Candidate[] }) {
  ...
  if (!open) {
    return (
      <button
        ...
        className="rounded-md bg-primary px-3 py-1.5 text-sm font-medium text-white hover:bg-primary-dark disabled:opacity-40"
  ...
  return (
    <form action={formAction} className="mt-3 flex flex-wrap items-end gap-3 rounded-md bg-surface p-3">
      ...
      <div>
        ...
        <select name="candidateId" required className="mt-1 rounded-md border border-border px-2 py-1 text-sm">
          {candidates.map((c) => (
      ...
      <div>
        ...
        <input name="proposedRate" type="number" step="0.01" required className="mt-1 w-28 rounded-md border border-border px-2 py-1 text-sm" />
      </div>
      <button
        type="submit"
        disabled={isPending}
